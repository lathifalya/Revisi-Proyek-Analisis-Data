...
selected_user_type = st.sidebar.multiselect("Pilih Tipe Penyewa", options=['casual', 'registered'], default=['casual', 'registered'])

# Filter data berdasarkan pilihan
filtered_df = day_df[day_df['season'].isin(selected_season)][['mnth', 'season'] + selected_user_type]

# Menghitung jumlah penyewa berdasarkan tipe per bulan dan musim
...
sns.barplot(data=kategori_penyewa, x='mnth', y=kategori_penyewa[selected_user_type].sum(axis=1), hue='season', ax=ax)
ax.set_xlabel("Bulan")
