...
# Menggunakan apply dengan lambda untuk pemetaan
day_df['season_name'] = day_df['season'].apply(lambda x: nama_musim.get(x, 'Unknown'))

# Sidebar untuk multi-select filter
st.sidebar.header("Filter Data")

# Menampilkan musim dalam bentuk nama bukan angka
selected_season = st.sidebar.multiselect(
    "Pilih Musim", 
    options=list(nama_musim.values()), 
    default=list(nama_musim.values())
)

# Memetakan nama musim kembali ke angka (season code)
selected_season_codes = [key for key, value in nama_musim.items() if value in selected_season]
...
filtered_df = day_df[day_df['season'].isin(selected_season_codes)]
...
# Menambahkan nama musim untuk visualisasi
kategori_penyewa['season_name'] = kategori_penyewa['season'].map(nama_musim)
...
for user_type in selected_user_type:
    sns.barplot(data=kategori_penyewa, x='mnth', y=user_type, hue='season_name', ax=ax)
